...
export default function (procFunctional, strict) {
  function _len(metadata, argv) {
    checkArity(metadata, argv, 1)
    argv = argv.map(strict)
    checkType(metadata, argv, AS.SequenceV)
  ...
  function _slice(metadata, argv) {
    checkArity(metadata, argv, [2, 3, 4])
    argv = argv.map(strict)
    checkType(metadata, argv[0], AS.SequenceV)
    checkType(metadata, argv.slice(1), AS.IntegerV)
  ...
  function _filter(metadata, argv) {
    checkArity(metadata, argv, 2)
    var seq = strict(argv[0])
    checkType(metadata, seq, AS.ListV)
    var fn = procFunctional(argv[1])
