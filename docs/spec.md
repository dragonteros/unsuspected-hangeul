...
## 다. 객체와 자료형
객체는 프로그램 실행의 기본 단위입니다. 객체는 `ㄱㄴㄷㄹㅁㅂㅅㅈ`으로만 단어를 구성한 정수 리터럴이거나, 함수 정의 혹은 호출로 만들어진 것이며, 인수로서 접근될 수 있습니다. 객체가 가질 수 있는 자료형은 다음 열한 가지입니다.
* 정수(Integer)
* 실수(Float)
* 복소수(Complex)
* 논릿값(Boolean)
...
예를 들어 인수에 상관 없이 정수 3을 돌려주는 함수는 `ㄹ ㅎ`과 같이 정의합니다.
...
인수에 상관 없이 정수 3을 돌려주는 함수에 두 개의 인수 1과 -1을 전달하여 호출하는 표현식은 다음과 같습니다.
...
## 바. 재귀 함수
함수를 정의할 때 정의하려는 함수 및 둘러싼 함수를 함수 몸통에서 호출할 수 있습니다. 정의하려는 함수를 0번째라고 하고, k번째 함수를 바로 둘러싼 함수를 (k+1)번째라고 합시다. m번째 함수를 접근하려면 다음과 같이 합니다.
...
m이 음수인 경우 바깥쪽에서 (-m-1)번째 함수를 접근합니다. 참고로 이 번호는 함수 객체 안에 환경으로 저장되어 함수 객체를 어느 문맥에서 호출하더라도 일관된 행동을 보장합니다.
...
## 자. 동적 인수 접근
몇 번째 인수를 접근할지 미리 정해지지 않으면 동적으로 값을 평가해 그 값으로 접근할 수 있습니다. 다음과 같이 정수 객체 하나와 `ㅇ`으로 시작하는 단어 하나로 이뤄진 표현식을 생각합시다. 정수 객체의 값이 n이면, 다음 표현식은 m번째 함수의 n번째 인수를 접근합니다.
